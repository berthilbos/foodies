                ...
                <div class=" col-lg-12 order-view" data-aos="zoom-in" data-aos-delay="100">
                    <div class="box" data-aos="zoom-in" data-aos-delay="100">
                        ...
                        <div hidden>
                            ...
                            <input type="text" class="restaurant" name="restaurant" id="restaurant"
                                value="{{ $product->restaurants->restaurant_name }}" style="display: none;" required>
                        </div>
                        <div hidden>
                            <label for="Product"></label>
                            <input type="text" class="product" name="product" id="product" value="{{ $product->food_name }}"
                                style="display: none;" required>
                        </div>
                        <div hidden>
                            <label for="price"></label>
                            <input type="text" class="price" name="price" id="price" value="{{ $product->price }}"
                                style="display: none;" required>
                        </div>
                        <div id="input_div">
                            <input type="number" value="1" name="quantity" id="quantity" min="1" max="25">
                            <p>{{ $errors->first('orderoverlay') }}</p>
                        </div>

                        <button class="btn btn-primary order-button is-link" type="submit">Confirm</button>
                        <button class="btn btn-primary order-button is-link" id="cancelorder"
                            formaction="/restaurants/{{ $product->restaurants->id }}"></a>Cancel</button>
                        </form>
                        </li>
                    </div>
                </div>
